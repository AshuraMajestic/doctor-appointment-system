...
function RelatedDoctor({ speciality, docId }) {  // ✅ Fixed function parameter

  const { doctors } = useContext(AppContext);
  const navigate = useNavigate(); // ✅ Fixed spelling

  const [relDoc, setRelDoc] = useState([]); // ✅ Fixed variable name

  useEffect(() => {
    if (doctors.length > 0 && speciality) {
      const doctordata = doctors.filter((doc) => doc.speciality === speciality && doc._id !== docId);
      setRelDoc(doctordata); // ✅ Fixed function name
    }
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      }} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10'>More Button</button>
    </div>
  )
}
